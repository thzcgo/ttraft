...
class NodeGroup {
     ...
     *
     * @return count
     * @see NodeGroupMember#isMajor()
     */
    int getCountOfMajor() {
        return (int) memberMap.values().stream().filter(NodeGroupMember::isMajor).count();
    }
    ...
    NodeGroupMember findMember(NodeId id) {
        NodeGroupMember member = getMember(id);
        if (member == null) {
    ...
    void downgrade(NodeId id) {
        ...
        NodeGroupMember member = findMember(id);
     ...
     * @return added member
     */
    NodeGroupMember addNode(NodeEndpoint endpoint, int nextIndex, int matchIndex, boolean major) {
        logger.info("add node {} to group", endpoint.getId());
        ReplicatingState replicatingState = new ReplicatingState(nextIndex, matchIndex);
        NodeGroupMember member = new NodeGroupMember(endpoint, replicatingState, major);
